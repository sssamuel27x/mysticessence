...
function setup(t, blockedSizes = [2]) {
  ...
  const documents = new Map([
    ...
    ['restockSubscriptions/sub', { productId: 'test', volume: '2ml', email: 'test@example.invalid', active: true, verifiedEmail: true, updatedAt: 'test-generation' }],
    ['restockSubscriptions/empty', { productId: 'test', volume: '5ml', email: 'test@example.invalid', active: true, verifiedEmail: true, updatedAt: 'test-generation' }],
  ]);
  ...
  t.mock.method(db, 'runTransaction', async callback => callback({
    get: reference => reference.get(), getAll: (...references) => Promise.all(references.map(reference => reference.get())),
    set: (reference, data) => reference.set(data), update: (reference, data) => reference.update(data),
  }));
...
test('a globally unavailable decant accepts a restock subscription', async (t) => {
  const state = setup(t);
  await subscribeToRestock.run({ rawRequest: { ip: '127.0.0.1' }, auth: { uid: 'test-user', token: { email: 'new@example.invalid', email_verified: true } }, data: { productId: 'test', volume: '2ml', email: 'new@example.invalid' } });
