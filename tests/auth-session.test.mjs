...
test("email and Google login both release busy state after success or failure", () => {
  const handlers = pageSource.slice(pageSource.indexOf('async function submitAccount'), pageSource.indexOf('if (!authReady)', pageSource.indexOf('async function submitAccount')));
  assert.equal((handlers.match(/finally\s*\{\s*setAuthBusy\(false\)/g) || []).length, 2);
});
