...
function setup(t, providerResponse = { Status: "000", RequestId: "test-request" }) {
  ...
  const collection = t.mock.method(db, "collection", (name) => ({
    doc: (id) => ({
      path: `${name}/${id}`,
      get: async () => ({ exists: documents.has(`${name}/${id}`), data: () => structuredClone(documents.get(`${name}/${id}`)) }),
  ...
  const transaction = t.mock.method(db, "runTransaction", async (callback) => callback({
    getAll: async (...refs) => refs.map(ref => ({ exists: documents.has(ref.path), data: () => structuredClone(documents.get(ref.path)) })),
...
test("card checkout does not require a Portuguese mobile number", async (t) => {
  const state = setup(t, { Status: "0", RequestId: "test-card", PaymentUrl: "https://hppgui.credorax.net/payment" });
  const result = await createCheckout.run(checkoutRequest("+34 612345678", "card"));
  assert.equal(result.method, "card");
  assert.equal(state.fetch.mock.callCount(), 1);
});

test("ambiguous provider failure keeps stock reserved for reconciliation", async (t) => {
  ...
  assert.equal(order.reconciliationRequired, true);
